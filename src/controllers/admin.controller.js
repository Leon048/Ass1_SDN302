...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const users = await User.find(query).select('-password_hash -refresh_token');
...
exports.createUser = async (req, res) => {
  try {
    const { email, password, full_name, phone, role, doctor_profile } = req.body;
...
exports.updateUser = async (req, res) => {
  try {
    const { full_name, phone, avatar_url, doctor_profile, owner_profile } = req.body;
    ...
    if (doctor_profile && user.role === 'doctor') user.doctor_profile = doctor_profile;
    if (owner_profile && user.role === 'owner') user.owner_profile = owner_profile;
...
exports.createInvoice = async (req, res) => {
  try {
    ...
    if (!appointment_id || !amount) {
      return res.status(400).json({ message: 'appointment_id and amount are required' });
    }
...
exports.getTopServices = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error retrieving top services', error: error.message });
  }
};
...
exports.getDoctorStats = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error retrieving doctor statistics', error: error.message });
  }
};
